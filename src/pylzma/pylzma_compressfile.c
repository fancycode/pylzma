...
pylzma_compfile_read(CCompressionFileObject *self, PyObject *args)
{
    ...
    if (length == self->outStream.size) {
        // All read
        self->outStream.data = NULL;
        self->outStream.size = 0;
    } else {
        // Compressed data remaining
        memmove(self->outStream.data, self->outStream.data + length, self->outStream.size - length); 
        self->outStream.size -= length;
    }
