    ...
    CCompressionMethodMode &headerMethod)
{
  ...
  if (_compressHeaders)
  {
    ...
    oneMethodInfo.MethodName = kLZMAMethodName;
    {
      ...
      property.Value.vt = VT_LPWSTR;
      property.Value.pwszVal = (LPWSTR)kMatchFinderForHeaders;
      oneMethodInfo.CoderProperties.Add(property);
    }
    {
      CProperty property;
      property.PropID = NCoderPropID::kAlgorithm;
      property.Value.vt = VT_UI4;
      property.Value.ulVal = kAlgorithmForX9;
      oneMethodInfo.CoderProperties.Add(property);
    }
    {
      CProperty property;
      ...
      property.Value.vt = VT_UI4;
      property.Value.ulVal = UINT32(kNumFastBytesForHeaders);
      oneMethodInfo.CoderProperties.Add(property);
    }
    {
      CProperty property;
      ...
      property.Value.vt = VT_UI4;
      property.Value.ulVal = UINT32(kDictionaryForHeaders);
      oneMethodInfo.CoderProperties.Add(property);
    }
  ...
  for(int i = 0; i < methodsInfo.Size(); i++)
  {
    ...
    if (IsLZMethod(oneMethodInfo.MethodName))
    {
      if (IsLZMAMethod(oneMethodInfo.MethodName))
      {
        PROPVARIANT prop;
        prop.vt = VT_UI4;
        prop.ulVal = _defaultDicSize;
        SetOneMethodProp(oneMethodInfo, 
            NCoderPropID::kDictionarySize, prop);
        prop.ulVal = _defaultAlgorithm;
        SetOneMethodProp(oneMethodInfo, 
            NCoderPropID::kAlgorithm, prop);
        prop.ulVal = _defaultFastBytes;
        SetOneMethodProp(oneMethodInfo, 
            NCoderPropID::kNumFastBytes, prop);
        prop.vt = VT_LPWSTR;
        const wchar_t *wtmp = _defaultMatchFinder;
        prop.pwszVal = (LPWSTR)wtmp;
        SetOneMethodProp(oneMethodInfo, 
            NCoderPropID::kMatchFinder, prop);
        if (multiThread)
        {
          prop.vt = VT_BOOL;
          prop.boolVal = true;
          SetOneMethodProp(oneMethodInfo, 
              NCoderPropID::kMultiThread, prop);
        }
      }
    }
...
HRESULT CHandler::SetParam(COneMethodInfo &oneMethodInfo, const UString &name, const UString &value)
{
  ...
  else
  {
    ...
    if (nameToPropID.VarType == VT_BSTR)
    {
      ...
      const wchar_t *wtmp = value;
      propValue.pwszVal = (LPWSTR)wtmp;
    } else
    {
      UINT32 number;
