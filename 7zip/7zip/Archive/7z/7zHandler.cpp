...
STDMETHODIMP CHandler::GetProperty(UINT32 index, PROPID propID,  PROPVARIANT *value)
{
  ...
  switch(propID)
  {
    case kpidPath:
    {
      ...
      utmp = NArchive::NItemName::GetOSName(item.Name);
      propVariant.pwszVal = (wchar_t *)utmp;
      break;
    }
    case kpidIsFolder:
      propVariant.vt = VT_BOOL;
      propVariant.boolVal = item.IsDirectory;
      break;
    case kpidSize:
      propVariant.vt = VT_UI8;
      propVariant.uhVal.QuadPart = item.UnPackSize;
      break;
    case kpidPackedSize:
    {
      {
        ...
        if (folderIndex >= 0)
        {
          ...
          if (_database.FolderStartFileIndex[folderIndex] == index)
            propVariant.uhVal.QuadPart = _database.GetFolderFullPackSize(folderIndex);
          else
            propVariant.uhVal.QuadPart = UINT64(0);
        }
        else
          propVariant.uhVal.QuadPart = UINT64(0);
      }
    ...
    case kpidAttributes:
      if (item.AreAttributesDefined)
      {
        ...
        propVariant.ulVal = item.Attributes;
      }
      break;
    case kpidCRC:
      ...
      {
        propVariant.vt = VT_UI4;
        propVariant.ulVal = item.FileCRC;
      }
      break;
    #ifndef _SFX
    case kpidMethod:
    ...
    case kpidBlock:
      {
        ...
        {
          propVariant.vt = VT_UI4;
          propVariant.ulVal = (UINT32)folderIndex;
        }
      }
      break;
    ...
    case kpidPackedSize4:
      {
        ...
        if (folderIndex >= 0)
        {
          ...
          {
            propVariant.uhVal.QuadPart = _database.GetFolderPackStreamSize(folderIndex, propID - kpidPackedSize0);
          }
          else
            propVariant.uhVal.QuadPart = UINT64(0);
        }
    ...
    case kpidIsAnti:
      propVariant.vt = VT_BOOL;
      propVariant.boolVal = item.IsAnti;
      break;
  }
  *value = propVariant;
